...
from states.search_states_lp import SearchFilterState
...
def lowprice(message: Message) -> None:
    """/lowprice command handler, which requires sending a message with needed city name and notifies owner of its usage(optional)"""
    if len(config.OWNER_ID) > 0:
        bot.send_message(config.OWNER_ID, ' Кто-то использовал команду: /lowprice')
    bot.set_state(message.from_user.id, SearchFilterState.city_lp, message.chat.id)
    bot.send_message(message.from_user.id, f'Здравствуйте {message.from_user.username}, введите город назначения')


@bot.message_handler(state=SearchFilterState.city_lp)
def get_city(message: Message) -> None:
    ...
    try:
        ...
        if len(city_name) < 1:
            ...
            bot.set_state(message.from_user.id, SearchFilterState.corrections_lp, message.chat.id)
...
@bot.message_handler(state=SearchFilterState.corrections_lp)
def get_corrections(message: Message) -> None:
    ...
    with bot.retrieve_data(message.from_user.id, message.chat.id) as data:
        ...
        data['city'] = city_name
    calendar, step = DetailedTelegramCalendar(min_date=datetime.date.today(), locale='ru', calendar_id=1).build()
    bot.send_message(message.chat.id,
                     f'Выберете дату заезда в отель:',
                     reply_markup=calendar)
    bot.set_state(message.from_user.id, SearchFilterState.arrival_date_lp, message.chat.id)
...
def cal(c: CallbackQuery):
    ...
    result, key, step_i = DetailedTelegramCalendar(min_date=datetime.date.today(), locale='ru', calendar_id=1).process(
        c.data)
...
@bot.message_handler(state=SearchFilterState.arrival_date_lp)
def get_arrival_date(message: Message) -> None:
    ...
    bot.set_state(message.from_user.id, SearchFilterState.deport_date_lp, message.chat.id)
...
def cal(c: CallbackQuery):
    ...
    result, key, step_i = DetailedTelegramCalendar(min_date=min_date, locale='ru', calendar_id=2).process(c.data)
    if not result and key:
...
@bot.message_handler(state=SearchFilterState.deport_date_lp)
def get_deport_date(message: Message) -> None:
    ...
    bot.set_state(message.from_user.id, SearchFilterState.price_range_lp, message.chat.id)


@bot.message_handler(state=SearchFilterState.price_range_lp)
def get_price_range(message: Message):
    ...
    if price_range[0].isdigit() and price_range[1].isdigit():
        ...
        bot.set_state(message.from_user.id, SearchFilterState.result_lp, message.chat.id)
...
@bot.message_handler(state=SearchFilterState.result_lp)
def get_result(message: Message) -> None:
    ...
    try:
        ...
        with bot.retrieve_data(message.from_user.id, message.chat.id) as data:
            ...
            parsed = [data['city'] + " lowprice"] + hotels
...
def send_photo_with_buttons(chat_id, index):
    """sends message with suiting hotels, with their description and photo"""
    ...
    btn_prev = keyboard_util.InlineKeyboardButton("←", callback_data=f"to {prev_index}")
    btn_current = keyboard_util.InlineKeyboardButton(f"{index + 1}/{total}", callback_data="current")
    btn_next = keyboard_util.InlineKeyboardButton("→", callback_data=f"to {next_index}")
